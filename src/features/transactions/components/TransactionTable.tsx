...
export function TransactionTable({ 
  ...
  const groupedData = React.useMemo(() => {
    ...
    for (const parent of parentAccounts) {
      ...
      const parentChildren = childAccounts.filter(child => child.parent_id === parent.id)

      const childGroups = parentChildren.map(child => {
        const childTxs = txsByAccount.get(child.id) || []
        const childTotalAmount = childTxs.reduce((sum, tx) => sum + (tx.amount || 0), 0)
        const childTotalPaid = childTxs.reduce((sum, tx) => sum + (tx.amount_paid || 0), 0)
        return {
          id: child.id,
          name: child.name,
      ...
      const directTotalAmount = directTxs.reduce((sum, tx) => sum + (tx.amount || 0), 0)
      const directTotalPaid = directTxs.reduce((sum, tx) => sum + (tx.amount_paid || 0), 0)

      const subAccountsTotalAmount = childGroups.reduce((sum, cg) => sum + cg.totalAmount, 0)
      const subAccountsTotalPaid = childGroups.reduce((sum, cg) => sum + cg.totalPaid, 0)
    ...
    const orphanGroups = orphans.map(child => {
      const childTxs = txsByAccount.get(child.id) || []
      const childTotalAmount = childTxs.reduce((sum, tx) => sum + (tx.amount || 0), 0)
      const childTotalPaid = childTxs.reduce((sum, tx) => sum + (tx.amount_paid || 0), 0)
      return {
  ...
  const unassignedTotalAmount = React.useMemo(() => {
    return groupedData.unassigned.reduce((sum, tx) => sum + (tx.amount || 0), 0)
  }, [groupedData.unassigned])

  const unassignedTotalPaid = React.useMemo(() => {
    return groupedData.unassigned.reduce((sum, tx) => sum + (tx.amount_paid || 0), 0)
  }, [groupedData.unassigned])
          ...
          {txs.length > 0 && (
            <tr className="border-b text-xs font-semibold text-muted-foreground/80 bg-card dark:bg-card/95">
              <td className="p-3 pl-4" colSpan={2}>
                <span className="uppercase tracking-wider text-[10px] font-bold text-muted-foreground">Totais do Período</span>
              </td>
              <td className="p-3 text-right font-bold text-foreground whitespace-nowrap w-24 sm:w-28">
                {formatCurrency(totalAmount)}
              </td>
              <td className={`p-3 text-center font-bold whitespace-nowrap w-40 sm:w-44 ${
                isEntrada 
                  ? 'text-emerald-600 dark:text-emerald-400' 
                  : 'text-rose-600 dark:text-rose-400'
              }`}>
                {isEntrada ? 'Recebido: ' : 'Pago: '}{formatCurrency(totalPaid)}
              </td>
            </tr>
          )}
        </thead>
              ...
              <>
                  ...
                  return (
                    <React.Fragment key={parent.id}>
                      ...
                      >
                        ...
                        <td className={`p-2.5 text-center text-xs font-bold whitespace-nowrap ${
                          isEntrada 
                            ? 'text-emerald-600 dark:text-emerald-400' 
                            : 'text-rose-600 dark:text-rose-400'
                        }`}>
                          {isEntrada ? 'Recebido: ' : 'Pago: '}{formatCurrency(parent.totalPaid)}
                        </td>
                      </tr>
                          ...
                          {parent.children.map((child) => {
                            ...
                            return (
                              <React.Fragment key={child.id}>
                                ...
                                >
                                  <td className="p-2 pl-8 text-xs text-foreground/75 whitespace-nowrap flex items-center gap-1.5" colSpan={2}>
                                    {childExpanded ? (
                                    ...
                                    <span className="font-semibold text-foreground/80 truncate">↳ {child.name}</span>
                                  </td>
                                  <td className="p-2 text-right font-semibold text-foreground/75 whitespace-nowrap">
                                    {formatCurrency(child.totalAmount)}
                                  </td>
                                  <td className={`p-2 text-center text-xs font-semibold whitespace-nowrap ${
                                    isEntrada 
                                      ? 'text-emerald-600/90 dark:text-emerald-400/90' 
                                      : 'text-rose-600/90 dark:text-rose-400/90'
                                  }`}>
                                    {isEntrada ? 'Recebido: ' : 'Pago: '}{formatCurrency(child.totalPaid)}
                                  </td>
                                </tr>

                                {/* Transações da Subconta */}
                                {childExpanded && child.txs.map((tx) => (
                                  <TransactionTableRow
                                    key={tx.id}
                                    tx={tx}
                                    isUpdating={updatingIds.has(tx.id)}
                ...
                {groupedData.unassigned.length > 0 && (
                  <>
                    ...
                    >
                      <td className="p-2.5 pl-4 text-xs font-bold text-muted-foreground whitespace-nowrap flex items-center gap-1.5" colSpan={2}>
                        {isExpanded('unassigned') ? (
                          <ChevronDownIcon className="h-4 w-4 text-muted-foreground shrink-0" />
                        ) : (
                          <ChevronRightIcon className="h-4 w-4 text-muted-foreground shrink-0" />
                        )}
                        <span className="font-bold text-muted-foreground truncate">Sem Conta</span>
                      </td>
                      ...
                      <td className="p-2.5 text-center text-xs font-bold text-muted-foreground whitespace-nowrap">
                        {isEntrada ? 'Recebido: ' : 'Pago: '}{formatCurrency(unassignedTotalPaid)}
                      </td>
                    </tr>
                    {isExpanded('unassigned') && groupedData.unassigned.map((tx) => (
                      <TransactionTableRow
                        key={tx.id}
                        tx={tx}
                        isUpdating={updatingIds.has(tx.id)}
