...
export function TransactionTableRow({
  ...
  return (
    <tr className={`border-b last:border-0 transition-colors group/row ${
      isLate 
      ...
      <td className={`whitespace-nowrap ${
        isSubChildRow ? 'p-2 pl-12 text-xs text-muted-foreground' : 
        isChildRow ? 'p-2 pl-8 text-xs text-muted-foreground' : 
        'p-4'
      }`}>
      ...
      <td className={`text-right whitespace-nowrap ${isThinner ? 'p-2 text-xs' : 'p-4'}`}>
      ...
      <td className={`text-right ${isThinner ? 'p-2' : 'p-4'}`}>
        <div className="flex items-center justify-end">
          {/* Status Area */}
          <div className={`${isThinner ? 'w-[100px] mr-2 h-6' : 'w-[110px] mr-3 h-7'} flex justify-end items-center`}>
            <PayButton 
              tx={tx}
